...
This might sound highly specific. But when one considers the fact that the scratch buffer calls the =prog-mode-hook=, it's easy to realize that any packages that are loaded in through a prog-mode-hook will be loaded in on start up. Making the prog-mode-hook counter intuitive to begin with.
